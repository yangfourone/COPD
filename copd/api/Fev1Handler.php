...
require_once('Fev1.php');
class Fev1Handler extends SimpleRest{
        ...
        switch($this->method){
            case 'get':
                if($this->action == 'getall'){
                    $fev1_all = new Fev1();
                    $this->set_status_code($this->encodeJson($fev1_all->getAll()));
                    break;
                }
                else if($this->action == 'getbyid'){
                    $fev1_id = new Fev1();
                    $this->set_status_code($this->encodeJson($fev1_id->getById($this->id)));
                    break;
                }
            case 'post':
                if($this->action == 'add'){
                    $fev1_add = new Fev1();
                    $this->set_status_code($this->encodeJson($fev1_add->add($this->input)));
                    break;
                }
            default:
