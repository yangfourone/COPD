...
if ($type=='PDF'){
	$endpoint = "localhost/copd/public/Download_Env_PDF.php?";
}
else {
	$endpoint = "localhost/copd/public/Download_Env_Excel.php?";
}
...
foreach($download_date as $value){
	//echo $value."<br>";
	$check_sql = "SELECT * FROM env WHERE datetime LIKE '$value %'";
...
if ($type=='PDF'){
	$files = array();
	foreach($download_date as $value){
		array_push($files,'download/Env '.$value.'.pdf');
	}
	$zipname = 'Env_PDF.zip';
}
else {
	$files = array();
	foreach($download_date as $value){
		array_push($files,'download/Env '.$value.'.xlsx');
	}
	$zipname = 'Env_Excel.zip';
}
